...
MODEL_TIME = numpy.zeros(1001,dtype = numpy.float)
MODEL_LAYER_TIME = numpy.zeros((1000,LAYER_NUM),dtype = numpy.float)
MODEL_CONVLAYER_TIME = numpy.zeros((1000,CONVLAYER_NUM),dtype = numpy.float)
MODEL_LAYER_TIME_P = numpy.zeros((1000,LAYER_NUM),dtype = numpy.float)
MODEL_CONVLAYER_TIME_P = numpy.zeros((1000,CONVLAYER_NUM),dtype = numpy.float)

CONV_INDEX = numpy.zeros(104,dtype = numpy.float)

#MODEL_LAYER_LIST = ["" for x in range(50)]
...
class EventList(list):
    ...
    def table(self, sort_by=None, row_limit=4000, header=None, top_level_events_only=False):
...
def build_table(
    ...
    global CONVLAYER_INDEX
    global SUM_MODLE_TIME,MODEL_INDEX,conv_index_n
    CONVLAYER_INDEX = 0
    SUM_MODLE_TIME = 0
    ...
    for evt in events:
        ...
        #MODEL_LAYER_LIST[i_index] = row_values[0]
        MODEL_LAYER_TIME_P[0][i_index] = row_values[3]
        MODEL_LAYER_TIME[0][i_index] = row_values[4]
        if row_values[0] == "aten::conv2d":
            MODEL_CONVLAYER_TIME[0][CONVLAYER_INDEX] = row_values[4]
            MODEL_CONVLAYER_TIME_P[0][CONVLAYER_INDEX] = row_values[3]
            #CONV_INDEX[conv_index_n] = i_index
            #conv_index_n = conv_index_n + 1
            CONVLAYER_INDEX = CONVLAYER_INDEX +1
        
        #print(i_index,MODEL_LAYER_LIST[i_index])
        i_index = i_index + 1
    ...
    MODEL_TIME[MODEL_INDEX] = float(format(format_time(self_cpu_time_total)))
    if MODEL_INDEX <= 998: 
        MODEL_INDEX = MODEL_INDEX + 1
    SUM_MODLE_TIME = SUM_MODLE_TIME + float(format(format_time(self_cpu_time_total)))
